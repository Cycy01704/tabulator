...
<div class="max-w-7xl mx-auto space-y-10">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">System Overview</h1>
            <p class="mt-2 text-slate-500 dark:text-slate-400 font-medium">Real-time data from your competition.</p>
        </div>
        <div class="flex items-center space-x-3 bg-white dark:bg-slate-900 p-2 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
            <span class="flex h-3 w-3 relative ml-2">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                <span class="relative inline-flex rounded-full h-3 w-3 bg-indigo-500"></span>
            </span>
            <span class="text-xs font-bold text-slate-600 dark:text-slate-300 pr-2">LIVE SYSTEM STATUS</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="relative group overflow-hidden bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline mt-2">
                    <p class="text-5xl font-extrabold text-slate-900 dark:text-white">{{ $stats['criteria'] }}</p>
        ...
        <div class="relative group overflow-hidden bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
            ...
            <div class="relative z-10">
                ...
                <div class="flex items-baseline mt-2">
                    <p class="text-5xl font-extrabold text-slate-900 dark:text-white">{{ $stats['judges'] }}</p>
                    <span class="ml-2 text-xs font-bold text-amber-600 dark:text-amber-400">ONLINE</span>
                </div>
            </div>
    ...
    @if(auth()->user()->isCommittee() && count($rankings) > 0)
        <!-- Ranking Insights Section -->
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
                <div class="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Real-time leaderboard analytics.</p>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 dark:bg-slate-800/50">
                                <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Rank</th>
                                <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">Participant</th>
                                <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 text-right">Avg Score</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach($rankings as $item)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                                    <td class="px-8 py-5">
                                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg @if($item->rank == 1) bg-amber-100 text-amber-600 @elseif($item->rank == 2) bg-slate-100 text-slate-600 @else bg-indigo-50 text-indigo-600 @endif font-display font-black text-xs">
                                            #{{ $item->rank }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-5">
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <span class="text-sm font-black text-indigo-600 dark:text-indigo-400">{{ number_format($item->average_score, 2) }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <!-- Additional Content Placeholder / Recent Activity -->
    <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div class="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">System Quick Start</h2>
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @if(auth()->user()->isJudge())
                <a href="{{ route('scoring.index') }}" class="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-transparent hover:border-indigo-500/20 hover:bg-white dark:hover:bg-slate-800 transition-all duration-300 group">
                    ...
                    <h4 class="font-bold text-slate-900 dark:text-white">Live Scoring</h4>
                    <p class="text-xs text-slate-500 mt-1">Start recording scores.</p>
                </a>
            @endif
