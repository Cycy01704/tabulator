...
<div class="max-w-3xl mx-auto space-y-10">
    <div class="flex items-center space-x-6">
        ...
        <div>
            <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit Access</h1>
            <p class="text-slate-500 font-medium">Modify permissions for <strong>{{ $user->name }}</strong>.</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <select name="role" id="role" required
                        ...
                        <option value="committee" {{ $user->role == 'committee' ? 'selected' : '' }}>Committee (Management Access)</option>
                        <option value="admin" {{ $user->role == 'admin' ? 'selected' : '' }}>System Admin (Full Authorization)</option>
                    </select>
                </div>

                <div class="space-y-3">
                    <label for="password" class="block text-sm font-bold text-slate-700 dark:text-slate-300 ml-1 uppercase tracking-widest">Security Pin (Leave blank to keep)</label>
